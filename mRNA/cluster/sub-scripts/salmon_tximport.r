...
# LINES CHANGED so that it only looks two levels down the directory tree
# Mainly because sometimes I keep old or discarded results inside path
fns <- list.dirs(path, recursive = F, full.names = T)
fns = list.files(fns, pattern = "quant.sf", recursive = F, full.names = T)
