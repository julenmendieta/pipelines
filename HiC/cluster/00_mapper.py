...
parser.add_argument('-r','--rEnzyme',help='Used Restriction Enzymes', required=False)
...
# get REs 
rEnzyme=args.rEnzyme
# if we provide an alternative output dir we change it 
if rEnzyme != None:
    # We will do fragment based mapping
    frag_map = True
    rEnzyme = rEnzyme.split('---')
...
if not os.path.exists(outTSV):
    ...
    if not os.path.exists(outMap):
        try:
            ...
            full_mapping(mapper_index_path=gem_index_path, 
                            ...
                            fastq_path=fi1, frag_map=frag_map, clean=True, nthreads=nthreads,
                            windows=rangePos,
                            temp_dir=temp_dir1, r_enz=rEnzyme)
            ...
            full_mapping(mapper_index_path=gem_index_path,
                            ...
                            fastq_path=fi2, frag_map=frag_map, clean=True, nthreads=nthreads,
                            windows=rangePos,
                            temp_dir=temp_dir2, r_enz=rEnzyme)
    ...
    if not os.path.exists(reads) and STOPnow == False:
        ...
        if rangePos != None:
            maps1 = [f'{outMap}mapped_{id1}_r1/{id11}_full_{p1}-{p2}.map'
                        for p1, p2 in rangePos]
            maps2 = [f'{outMap}mapped_{id1}_r2/{id22}_full_{p1}-{p2}.map'
                        for p1, p2 in rangePos]
        else:
            maps1 = [f'{outMap}mapped_{id1}_r1/{id11}_frag_1-end.map']
            maps2 = [f'{outMap}mapped_{id1}_r2/{id22}_frag_1-end.map']
        ...
        parse_map(maps1, maps2,
                    reads1, 
                    reads2, 
                    genome_seq=genome_seq, re_name=rEnzyme, 
                    verbose=True)
        ...
        #_ = plot_distance_vs_interactions(reads, resolution=100000, max_diff=1000, 
        #                                show=False, savefig=pathFinal + 'distanceDecay_%s.pdf' %id1)

        # Get genomic coverage
        _ = plot_genomic_distribution(reads, resolution=500000, ylim=(0, 8000), 
                                    show=False, 
                                    savefig=pathFinal + 'genomicCoverage_%s.pdf' %id1)
    ...
    if STOPnow == False:
        ...
        if not os.path.exists(outTSV):
            ...
            # Plot insert sizes
            max_molecule_length = insert_sizes(reads, show=False, 
                                                nreads=1000000,
                                                savefig=pathFinal + 'insertSizes_%s.pdf' %id1)[1]

            ## Filtering mapped reads
            min_dist_to_re = max_molecule_length * 1.5 
            print(max_molecule_length, min_dist_to_re)
