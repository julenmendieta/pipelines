...
option_list <- list(make_option(c("-i", "--featurecount_file"), type="character", default=NULL, help="Feature count file generated by the SubRead featureCounts command.", metavar="path"),
                    ...
                    make_option(c("-y", "--bybatch"), type="logical", default=FALSE, help="only compare by same batch", metavar="boolean"),
...
# ME QUEDO EN AÑADIR AQUI LA INFO DE LOS BATCHES PARA MAS ADELANTE
...
for (batch in batches) {
    ResultsFile <- paste(opt$outprefix,".results_", batch, ".txt",sep="")
    if (file.exists(ResultsFile) == FALSE) {
        ...
        for (idx in 1:ncol(comparisons)) {
            ...
            kos = unlist(lapply(comparisons[,idx], function (x) {
                    unlist(strsplit(
                        unlist(strsplit(x, '-'))[2]
                        , '_'))[1]}))
            # We only want to compare KO vs Controls
            if (sum(kos %in% controls) == 1) {
                ...
                deseq2_results_list[[idx_]] <- comp.df
                idx_ = idx_ + 1
            }
        }
